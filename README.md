...
おおまかに 4 段階に分けて取り組みました。

### 1. 2022 年の卒業研究のコードを今のスタックで再現する

当時の TensorFlow + Keras の実装を、Python 3.12 / TF 2.21 で動くように移植したうえで、6 種類のアルゴリズム (CNN, BP NN, KNN, Naive Bayes, SVM, Decision Tree) の比較を再実行しました。再現してみたところ、当時のラベル前処理に「`N` の一括置換が `FTP-CONTROL` と `INTERACTIVE` のクラス名まで壊し、これら 2 クラスのサンプルが完全に欠落する」というバグが紛れていたことに気付き、修正しました。4 年前の自分が見落とした地味な不具合を、自分で見つけて潰す作業から始まりました。
