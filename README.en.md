...
2. **A 4-year-later self-review extension** — PyTorch GPU, 1D CNN, Dilated Conv, and a systematic study of how batch size affects accuracy.

The main result is that ATTACK class F1 improved from about 57% to 79.89%. The key turned out to be picking the right batch size, not exotic loss functions.
...
Full instructions: [USAGE.md](USAGE.md). Experiment notes (motivation, implementation, results for each improvement): [EXPERIMENTS.md](EXPERIMENTS.md).
